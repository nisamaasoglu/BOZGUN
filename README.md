...
![BOZGUN demo — synthetic target mode](docs/demo.gif)

*`python bozgun.py --demo` — the full detect → track → lock-on → hit pipeline running with no webcam and no Arduino.*

![BOZGUN running on the real rig](docs/bozgun_interface.jpg)

*Live colour tracking on the physical turret — lock-on in progress.*
